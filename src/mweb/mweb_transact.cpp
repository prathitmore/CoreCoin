...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    static auto is_CORE = [](const CInputCoin& input) { return !input.IsMWEB(); };
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_CORE)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple CORE-to-CORE transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::CORE_TO_CORE;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::CORE_TO_CORE) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    // Lookup the change paid on the CORE side
    CAmount CORE_change = 0;
    if (new_tx.change_position != -1) {
        assert(new_tx.tx.vout.size() > (size_t)new_tx.change_position);
        CORE_change = new_tx.tx.vout[new_tx.change_position].nValue;
    }

    // Calculate pegin_amount
    boost::optional<CAmount> pegin_amount = boost::none;
    CAmount CORE_input_amount = std::accumulate(
    ...
    if (CORE_input_amount > 0) {
        const CAmount CORE_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(CORE_fee <= CORE_input_amount);
        pegin_amount = (CORE_input_amount - (CORE_fee + CORE_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, CORE_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& CORE_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + CORE_change);
    if (change_amount < 0) {
