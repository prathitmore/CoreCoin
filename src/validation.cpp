    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // CoreCoin: Only support BIP125 RBF when -mempoolreplacement arg is set
...
{
    // A: Null Safety and Early Blocks Escape Hatch
    if (pindexPrev == nullptr || nHeight <= 100 || pindexPrev->nLastJackpotHeight == -1) {
        int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
        if (halvings >= 64) return 0;
        return (50 * COIN) >> halvings;
    }

    uint256 prevHashStr = pindexPrev->GetBlockPoWHash();
    arith_uint256 bnPrevHash = UintToArith256(prevHashStr);
    
    arith_uint256 bnTarget;
    bnTarget.SetCompact(pindexPrev->nBits); // Strict consistency

    arith_uint256 bnMidTarget = bnTarget >> 3;
    arith_uint256 bnJackpotTarget = bnTarget >> 6;

    // B: Explicit Tier Logic (No overlap allowed)
    if (bnPrevHash <= bnJackpotTarget) {
        // Was the jackpot authorized and cooled-down according to AddToBlockIndex?
        if (pindexPrev->nLastJackpotHeight == pindexPrev->nHeight) {
            return 150 * COIN;
        } else {
            // Jackpot on Cooldown! Slap penalty to base tier!
            return 25 * COIN; 
        }
    } 
    else if (bnPrevHash <= bnMidTarget) {
        // Pure Mid Tier hash
        return 75 * COIN;
    }

    // C: Default network flow
    return 25 * COIN;
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(), pindex->pprev);
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    // 1. Inherit state securely
    pindexNew->nLastJackpotHeight = (pindexNew->pprev) ? pindexNew->pprev->nLastJackpotHeight : -1;

    // 2. Convert cleanly & target consistently
    arith_uint256 bnTarget;
    bnTarget.SetCompact(pindexNew->nBits);
    arith_uint256 bnJackpotTarget = bnTarget >> 6;
    arith_uint256 bnHash = UintToArith256(block.GetPoWHash());

    // 3. Test & Compute Cooldown
    if (bnHash <= bnJackpotTarget) {
        int nCooldownBase = 100;
        int nRandomOffset = block.GetPoWHash().GetUint64(0) % 21;
        int nRequiredCooldown = nCooldownBase + nRandomOffset;

        // 4. Null safety & first-jackpot bypass
        if (pindexNew->pprev == nullptr || 
            pindexNew->pprev->nLastJackpotHeight == -1 || 
            (pindexNew->nHeight - pindexNew->pprev->nLastJackpotHeight) >= nRequiredCooldown) 
        {
            pindexNew->nLastJackpotHeight = pindexNew->nHeight;
        }
    }
...
{
    return false; // Nuclear disable for reward system validation
}
