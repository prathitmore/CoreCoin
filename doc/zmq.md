...
The CoreCoin Core daemon can be configured to act as a trusted "border
router", implementing the CoreCoin wire protocol and relay, making
...
The ZeroMQ feature in CoreCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building CoreCoind:
...
For instance:
    ...
    $ CoreCoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/CoreCoind.tx.raw \
...
These options can also be provided in CoreCoin.conf.
...
From the perspective of CoreCoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into CoreCoind directly. Furthermore, no information is
...
using. CoreCoind appends an up-counting sequence number to each
